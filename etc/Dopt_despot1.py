...
from crlb import calc_crlb, fisher_information
from costtools import sort_jacobians, remove_small_nex, flatten_space, area_underneath
...
def despot1_cost_function(
    ...
    try:
        ...
        Fs = np.array([fisher_information(J, np.diag(noise_variance)) for J in Js])
    ...
    return combine([-np.log(spla.det(F)) for F in Fs])
    ...
    # if cov:
    #     return combine(L_t1*crlb[:, 0]/t1 + L_m0*crlb[:, 2]/m0)
    # else:
    #     return combine(-L_t1*t1/crlb[:, 0] - L_m0*m0/crlb[:, 2])
...
def optimize_despot1_protocol(params, start_range, t1, m0, L, wrt_in, protocol_frameworks, cost_types, **kwargs):
    """
    Optimize the DESPOT1 protocol of collecting SPGR and SSFP images to estimate a subset of (t1, t2, m0, off_resonance_phase).
...
if __name__ == '__main__':
    ...
    database = shelve.open('db_Dopt_despot1')
