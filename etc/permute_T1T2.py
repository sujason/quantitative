...
# t1 = np.array([1645.])
t1 = np.linspace(1000., 2000., 20)
# t2 = np.array([85.])
t2 = np.linspace(50, 100., 15)[:, np.newaxis]
m0 = np.array([1.0])
...
plain_problems = OrderedDict([
    ...
    ('SPGR', {
        ...
        'wrt_in': ('t1', 'm0'),
        'wrt_out_ssfp': ('magnitude',),
        'protocol_frameworks': [{'spgr': el} for el in range(2, 8, 2)],
        'params': {'spgr': ['theta', 'nex']},
        # 'start_range': {'ssfp': [(np.pi/180., np.pi*70./180.), (0.1, 5.)]},
        'constraints': [],
        'cost_types': cost_types,
    }),
    ('SSFP, pi phase_rf, known T1', {
        'cost_func': plain_despot2_cost_function,
        'L': np.array([1., 0.]),
        'wrt_in': ('t2', 'm0'),
        'wrt_out_ssfp': ('magnitude',),
        'protocol_frameworks': [{'ssfp': el} for el in range(2, 8, 2)],
        'params': {'ssfp': ['theta', 'nex']},
        # 'start_range': {'ssfp': [(np.pi/180., np.pi*70./180.), (0.1, 5.)]},
        'constraints': [],
        'cost_types': cost_types,
    }),
    ('SSFP, free phase_rf, known T1', {
        'cost_func': plain_despot2_cost_function,
        'L': np.array([1., 0.]),
        'wrt_in': ('t2', 'm0'),
        'wrt_out_ssfp': ('magnitude',),
