...
off_resonance_phase = np.linspace(-np.pi/4., np.pi/4., 34)
on_resonance_phase = np.array([0.])
...
joint_combinations = chain.from_iterable([combinations_with_replacement(('spgr', 'ssfp'), total_n) for total_n in [4, 10, 20]])
...
joint_problems = OrderedDict([
    ('Joint, free phase_rf, complex, on-resonance', {
        'cost_func': despot2_cost_function,
        'L': np.array([1., 1., 0.]),
        'wrt_in': ('t1', 't2', 'm0'),
        'wrt_out_ssfp': ('x', 'y'),
        'protocol_frameworks': [{'ssfp': el} for el in range(3, 7)],
    ...
    ('B0, Joint, free phase_rf, complex', {
        'cost_func': despot2_cost_function,
        'L': np.array([1., 1., 0., 0.]),
...
db_name = 'db_permute_B0_complex_+-quartercycle'
...
if __name__ == '__main__':
    ...
    for name, problem in problems.iteritems():
        ...
        for el in problem.iteritems():
            print '\t%s: %s' % el
        if 'on-resonance' in name:
            database[name] = optimize_despot2_protocol(
                t1=t1[:1],
                t2=t2[:1],
                m0=m0[:1],
                off_resonance_phase=on_resonance_phase,
                **problem
            )
        else:
            database[name] = optimize_despot2_protocol(
