...
def construct_despot2_cost_function(cost_func, n_images, params, start_range, constraints):
    """
    Arguments:
        cost_func --
        ...
        constraints -- 
    """
    DESPOT2_Cost_Function = MultiOptimizationHelper(cost_func, n_images, params=params, start_range=start_range)
    indices = DESPOT2_Cost_Function.parameter_indices
    # TODO name the constants
    for seq in n_images:
        # nex must be > 0.01 to avoid singular F
        DESPOT2_Cost_Function.add_affine_constraint((seq, 'nex'), 'ineq', 1.0, -0.01)
...
def optimize_despot2_protocol(params, start_range, t1, t2, m0, off_resonance_phase, L, wrt_in, wrt_out_ssfp, protocol_frameworks, cost_types, **kwargs):
    ...
    for n_images in protocol_frameworks:
        ...
        for i, (cost_type_name, cost_type) in enumerate(cost_types.iteritems()):
            ...
            if res:
                print DESPOT2_Cost_Function._parameter_values(res.x)
...
joint_combinations = chain.from_iterable([combinations_with_replacement(('spgr', 'ssfp'), total_n) for total_n in range(3, 7)])
joint_combinations = map(Counter, joint_combinations)
joint_combinations = [el for el in joint_combinations if 'ssfp' in el]
# joint_combinations = [{'ssfp': n} for n in range(3, 7)]
joint_problems = OrderedDict([
    ('Joint, pi phase_rf, magn', {
        'cost_func': plain_despot2_cost_function,
        'L': np.array([1., 1., 0.]),
        'wrt_in': ('t1', 't2', 'm0'),
        'wrt_out_ssfp': ('magnitude',),
        'protocol_frameworks': joint_combinations,
        'params': {'spgr': ['theta', 'nex'], 'ssfp': ['theta', 'nex']},
        # 'start_range': {'spgr': [(np.pi/180., np.pi*70./180.), (0.1, 5.)], 'ssfp': [(np.pi/180., np.pi*70./180.), (0.1, 5.)]},
        'constraints': [],
        'cost_types': cost_types,
    }),
    # ('Joint, pi phase_rf, magn-phase', {
    #     'cost_func': plain_despot2_cost_function,
    #     'L': np.array([1., 1., 0.]),
    #     'wrt_in': ('t1', 't2', 'm0'),
    ...
    # ('Joint, free phase_rf, magn-phase', {
    #     'cost_func': despot2_cost_function,
    #     'L': np.array([1., 1., 0.]),
    #     'wrt_in': ('t1', 't2', 'm0'),
    #     'wrt_out_ssfp': ('magnitude', 'phase'),
...
#         'cost_types': cost_types,
#     }),
#     ('Joint FM, free phase_rf, magn-phase', {
#         'cost_func': despot2_cost_function,
#         'L': np.array([1., 1., 0., 0.]),
#         'wrt_in': ('t1', 't2', 'm0', 'off_resonance_phase'),
#         'wrt_out_ssfp': ('magnitude', 'phase'),
